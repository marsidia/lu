...
function TextInput({primary, secondary, err, children, onChange, getValue}){
    ...
    return(
        <div ref = {divEl} className="text-input">
            ...
            <input placeholder={children} className={classInput}  type="text" value={entry} onChange= {handleChange} onClick={() => setIsOpen(true)}/
>           
            <label className={classLabel}>{children}</label>  
            <div className={classErrMsg}>Please select a valid {children}</div> 
            
            
            {isOpen && suggData && <SuggList isLoading = {results.isFetching || results.isLoading} suggData={suggData} setEntry={handleSetEntry}/>}
        </div>
    )
}
