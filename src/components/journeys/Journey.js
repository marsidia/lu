      ...
      const geodata = journey.sections.reduce((res, section) => {
        if (section.geojson) {
          res.push({
            ...
            properties: {
              color: section.display_informations
  ...
  return (
    <div className="journey" onClick={handleClick}>
      <div className="journey__info" onClick={() => setIsOpen(!isOpen)}>
        <div className="journey__info__time">
          {getDisplayDuration(journey.duration)}
        </div>
      ...
      {true && (
        <SectionsList
          className={
            isOpen
              ? "journey__section-list--open"
              : "journey__section-list--closed"
          }
